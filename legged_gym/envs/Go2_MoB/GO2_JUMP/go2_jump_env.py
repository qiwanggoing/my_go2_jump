...
class GO2_JUMP_Robot(BaseTask):
    def __init__(self, cfg: GO2_JUMP_Cfg_Yu, sim_params, physics_engine, sim_device, headless):
    ...
    def reset_idx(self, env_ids):
        ...
        # self.reset_buf[env_ids] = 1 #????是为1的时候才重置吗还是为0的时候重置
    ...
    def _reward_tracking_lin_vel(self):
        ...
        reward=torch.exp(-lin_vel_error/self.cfg.rewards.tracking_sigma)*(torch.norm(self.commands[:, :2], dim=1) > 0.2)
        reward+=torch.exp(-torch.norm(self.base_lin_vel[:, :2])/self.cfg.rewards.tracking_sigma)*(torch.norm(self.commands[:, :2], dim=1) < 0.2)
        # print(reward.shape)
        return reward
    ...
    def _reward_tracking_ang_vel(self):
        ...
        # print(ang_vel_error.shape,(torch.abs(self.commands[:, 2]) > 0.2).shape)
        reward =torch.exp(-ang_vel_error/self.cfg.rewards.tracking_sigma)*(torch.abs(self.commands[:, 2]) > 0.2)
        reward+=torch.exp(-torch.abs(self.base_ang_vel[:, 2])/self.cfg.rewards.tracking_sigma)*(torch.abs(self.commands[:, 2]) < 0.2)
        return reward
